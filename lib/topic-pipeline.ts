...
export function validatePipelineStep(examType: ExamType, input: PipelineStepInput): void {
  ...
  const courseAllowed = examType === "LGS" ? isLgsCourseId(input.courseId) : yksCourseIds().has(input.courseId);
