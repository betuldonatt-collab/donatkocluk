...
import { setTaskStatus, type TaskScope, type TaskStatus } from "./actions";
...
function withDefaults(tasks: Task[], initial: Record<string, TaskState>) {
  return Object.fromEntries(
    tasks.map((t) => [t.id, initial[t.id] ?? { status: "pending" as const }]),
  );
}

export function OdevlerClient({
  initialDailyState,
  initialWeeklyState,
}: {
  initialDailyState: Record<string, TaskState>;
  initialWeeklyState: Record<string, TaskState>;
}) {
  const [activeTab, setActiveTab] = useState<"daily" | "weekly">("daily");
  const [dailyState, setDailyState] = useState(() => withDefaults(DAILY_TASKS, initialDailyState));
  const [weeklyState, setWeeklyState] = useState(() => withDefaults(WEEKLY_TASKS, initialWeeklyState));
  ...
  function updateTask(id: string, patch: Partial<TaskState>) {
    const next = { ...taskState[id], ...patch };
    setTaskState((prev) => ({ ...prev, [id]: next }));
    void setTaskStatus({ scope, taskId: id, ...next });
  }

  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      <header className="mb-6">
        <h1 className="text-2xl font-semibold text-foreground">Ödevler</h1>
...
function TaskCard({
  ...
  const [draftReason, setDraftReason] = useState<string | undefined>();
  ...
  function saveModal() {
    onChange({
      status: "not_done",
      reason: draftReason,
      note: draftReason === "Diğer" ? draftNote : undefined,
    });
    setModalOpen(false);
  }
    ...
    <Card>
      ...
      <CardContent>
        <div className="flex flex-wrap items-center gap-2">
          ...
          {state.status === "not_done" && state.reason && (
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Neden yapamadın?</DialogTitle>
          ...
          <div className="flex flex-wrap gap-2">
            {EXCUSE_REASONS.map((reason) => (
              <button
                key={reason}
                type="button"
                aria-pressed={draftReason === reason}
          ...
          <DialogFooter>
            ...
            <Button type="button" disabled={!draftReason} onClick={saveModal}>
              Kaydet
            </Button>
          </DialogFooter>
        </DialogContent>
