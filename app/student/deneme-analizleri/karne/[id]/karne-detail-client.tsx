...
// Mirrors the coach panel's own TotalDurationCard (karneler-tab.tsx)
// exactly. Every minute logged in the system this cycle
// (NetSummary.totalDurationMinutes) -- across every task type, not just
// the 4 exam-shaped ones ScoreBreakdownCard covers. Undefined (not 0) for
// a cycle generated before this field existed -- the caller only renders
// this when it's actually present.
...
// Mirrors the coach panel's own ScoreBreakdownCard (karneler-tab.tsx)
// exactly, duplicated per this codebase's per-panel UI convention.
// Replaces the old Soru Çözüm Hızı (questions/hour) card with the cycle's
// actual Doğru/Yanlış/Boş totals -- a grand total row (led by an explicit
// Toplam Soru pill, not just the D/Y/B split) plus a per-subject/course
// breakdown underneath, each row also stating its own Toplam Soru rather
// than leaving the reader to add D+Y+B up themselves. Shared by the TYT
// card (stats.scoreBreakdown) and each AYT track's own card (one per
// stats.aytScoreBreakdown entry).
    ...
    <div className="space-y-6 print:space-y-4">
      ...
      {coachNotes && (
        <section className="border-border bg-card space-y-2 rounded-lg border p-4 print:break-inside-avoid">
          <h3 className="text-foreground text-sm font-semibold">Koçundan Not</h3>
