...
// inline "add to library" confirmation instead of silently discarding
// it or forcing a separate creation step.
export function ResourceCombobox({
  ...
  // Archived resources (Group 4a) stay resolvable by id/name for a task
  // that's already linked to one (see valueFromTask, task-form-fields.tsx
  // -- it looks the name up from the SAME unfiltered `resources` list this
  // component receives), but never show up as a pickable option for a NEW
  // link -- that's the entire point of archiving one.
    ...
    <div ref={containerRef} className="relative space-y-1.5">
          ...
          <div className="max-h-48 overflow-y-auto py-1">
            {filtered.length === 0 ? (
              <p className="text-muted-foreground px-3 py-2 text-sm">
                {pickableResources.length === 0 ? "Öğrenci bu ders için henüz kaynak eklemedi." : "Eşleşen kaynak yok."}
              </p>
            ) : (
              filtered.map((r) => (
                <button
      ...
      {isNew && (
        <label className="text-muted-foreground flex items-start gap-2 text-xs">
          ...
          <span>Bu kaynak öğrencinin kütüphanesinde yok, kütüphaneye eklensin mi?</span>
        </label>
      )}
    </div>
  );
